...
class BiasFree_LayerNorm(nn.Module):
    def __init__(self, normalized_shape):
        ...
        if isinstance(normalized_shape, numbers.Integral): ##pan duan n_s shi fou wei zheng xing
    ...
    def forward(self, x):
        ...
        return x / torch.sqrt(sigma + 1e-5) * self.weight ##zhu yuan su ji suan ping fang gen
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        x = self.project_in(x)
        x1_3, x2_3 = self.relu3(self.dwconv3x3(x)).chunk(2, dim=1) #通过chunk方法在维度1上分为两份
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(attn.size()) ##torch.Size([1, 1, 48, 48])
        # assert 1==0
...
#The following context is used in GroupOLs
class OperationLayer(nn.Module):
    def __init__(self, C, stride):
        super(OperationLayer, self).__init__()
        self._ops = nn.ModuleList() # motive?
        for o in Operations:
            op = OPS[o](C, stride, False) #OPS in the back
            self._ops.append(op)
...
#The GroupOLs is used in the MEFC
...
#The following context is used in MEFC
class OALayer(nn.Module):
    def __init__(self, channel, k, num_ops):
        ...
        # self.max_pool = nn.AdaptiveMaxPool2d(1) #be used in our model
    ...
    def forward(self, x):
        # y1 = self.avg_pool(x)
        # y2 = self.max_pool(x)
        # y1 = y1.view(x.size(0), -1)
        # y2 = y2.view(x.size(0), -1)
        # y1 = self.ca_fc(y1)
        # y2 = self.ca_fc(y2)
        # y = y1 + y2
...
#         self.op = nn.Sequential(
#             nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=stride, padding=padding, groups=C_in, bias=False),# each input channel is convolved with its own set of filters (c_out/c_in)
#             nn.Conv2d(C_in, C_in, kernel_size=1, padding=0, bias=False),
#             nn.ReLU(inplace=False),
#             nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=1, padding=padding, groups=C_in, bias=False),#motive?
#             nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False),)
...
class SepConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True):
        ...
        self.conv1 = nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=stride, padding=padding, groups=C_in, bias=False)
        self.conv12 = nn.Conv2d(C_in, C_in, kernel_size=1, padding=0, bias=False)
        self.conv13 = nn.ReLU(inplace=False)
        self.conv2 = nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=1, padding=padding, groups=C_in, bias=False)
        self.conv22 = nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False)
        # self.sigmoid = nn.Sigmoid()
        # self.op = nn.Sequential(
...
class DRSformer(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        print('inp_1', inp_enc_level1.size())
        ...
        print('out_2', out_dec_level1.size())
